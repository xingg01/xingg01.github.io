const NaoKuo = {
    ...
    try {
      if (!ipLoacation) {
        return new Promise((resolve, reject) => {
          ...
          var url = `https://apis.map.qq.com/ws/location/v1/ip?key=2DIBZ-GCS6J-IJ2FB-XLQ54-HBQC3-G2FUT&output=jsonp`;
          script.src = url;
  ...
  showWelcome: (ipStore) => {
    ...
    let dist = NaoKuo.getDistance(26.881037, 112.677173, ipStore.result.location.lng, ipStore.result.location.lat),
      address,
      welcome_info;
    //根据国家、省份、城市信息自定义欢迎语
    //海外地区不支持省份及城市信息
